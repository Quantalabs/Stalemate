    ...
    for x in range(1, 9):
        for y in range(1, 9):
            if board[(x - 1) * 8 + y - 1] == 0:
                ...
                piece = c.create_text(50 * y + 200 - 25, 200 + 25 + x * 50 - 50, text=board[(x - 1) * 8 + y - 1],
                                      font=("Helvetica", 30))
    root.mainloop()


def game(game, moves=10):
    root = Tk()
    root.title('Stalemate')
    c = Canvas(width=800, height=800)
    c.pack()
        ...
        for x in range(1, 9):
            for y in range(1, 9):
                if board[(x - 1) * 8 + y - 1] == 0:
                    ...
                    piece = c.create_text(50 * y + 200 - 25, 200 + 25 + x * 50 - 50,
                                          text=board[(x - 1) * 8 + y - 2],
                                          font=("Helvetica", 30))
    ...
    def redraw(game):
        for board in game[scene]:
            ...
            for x in range(1, 9):
                for y in range(1, 9):
                    if board[(x - 1) * 8 + y - 2] == 0:
                        continue
                    else:
                        piece = c.create_text(50 * y + 200 - 25, 200 + 25 + x * 50 - 50,
                                              text=board[(x - 1) * 8 + y - 1],
                                              font=("Helvetica", 30))

    def push_scene(scene_var):
        if scene+1 > moves:
            scene_var += 1
        else:
            pass

    def Next(scene_var, board):
        push_scene(scene_var)
        redraw(board[scene_var])
        root.canvas.update_idletasks()

    nextButton = Button(root, text="Next", command=Next(scene, game))
    nextButton.place(x=850, y=900)
